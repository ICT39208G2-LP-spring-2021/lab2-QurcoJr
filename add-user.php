...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    (empty($_POST['FirstName'])) ? $FirstNameError = 'Name is required':$FirstName = $_POST['FirstName'];

    (empty($_POST['LastName'])) ? $LastNameError = 'Surname is required': $LastName = $_POST['LastName'];

    (empty($_POST['PersonalNumber'])) ? $PersonalNumberError = 'Personal ID is required' : $PersonalNumber = $_POST['PersonalNumber'];
    ...
    (empty($_POST['StatusId'])) ? $StatusIdError = 'Status ID is required':$StatusId = $_POST['StatusId'];
}
...
if (!(isset($FirstNameError, $LastNameError, $PersonalNumberError, $EmailError, $PasswordError, $StatusIdError))) {
    ...
    if ($existingPersonalNumber) {
        $PersonalNumberError = 'personal ID is already taken';
        $hasError = true;
    }
}
